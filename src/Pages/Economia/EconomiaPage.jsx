...
const EconomiaPage = () => {
  ...
  useEffect(() => {
    const cargarNoticias = async () => {
      try {
        console.log('📥 Cargando noticias de economía...');
        
        const q = query(
          collection(db, 'noticias'),
          where('categoria', '==', 'Economía'),
        ...
        console.log('✅ Noticias de economía cargadas:', noticiasData.length);
        setNoticias(noticiasData);
      } catch (error) {
        console.error('❌ Error cargando noticias de economía:', error);
      } finally {
        setCargando(false);
      }
    };
  ...
  if (cargando) {
    return (
      <div className="categoria-loading">
        ...
        <p>Cargando noticias de economía...</p>
      </div>
    );
  }
  ...
  return (
    <div className="categoria-page">
      <div className="categoria-header">
        <div className="categoria-hero economia-hero">
        ...
        <div className="search-bar">
          <input
            type="text"
            placeholder="Buscar en economía..."
            value={busqueda}
      ...
      <div className="categoria-content">
        ...
        {noticiasFiltradas.length === 0 ? (
          <div className="no-noticias">
            ...
            <h3>No hay noticias de economía</h3>
            <p>
              {busqueda 
                ? 'No se encontraron noticias que coincidan con tu búsqueda.'
                : 'No hay noticias de economía publicadas aún.'
              }
            </p>
          </div>
        ) : (
              ...
              <article key={noticia.id} className="noticia-card">
                {noticia.imagen && (
                  <div className="noticia-imagen">
                    ...
                    <div className="noticia-categoria">Economía</div>
                  </div>
                )}
                ...
                <div className="noticia-content">
                  ...
                  <h3 className="noticia-titulo">
                    <Link to={`/economia/${noticia.id}`}>{noticia.titulo}</Link>
                  </h3>
                  ...
                  <Link to={`/economia/${noticia.id}`} className="leer-mas">
                    Leer más →
                  </Link>
                </div>
              </article>
