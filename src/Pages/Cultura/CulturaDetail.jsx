...
const CulturaDetail = () => {
  ...
  useEffect(() => {
    const cargarNoticia = async () => {
      try {
        console.log('📥 Cargando noticia individual de cultura:', id);
        const docRef = doc(db, 'noticias', id);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const noticiaData = { id: docSnap.id, ...docSnap.data() };
          console.log('✅ Noticia de cultura cargada:', noticiaData.titulo);
          
          // Verificar que la noticia sea de la categoría correcta
          if (noticiaData.categoria !== 'Cultura') {
            console.log('❌ Noticia no pertenece a cultura');
            setNoticia(null);
          } else {
  ...
  if (cargando) {
    return (
      <div className="categoria-detail-loading">
        ...
        <p>Cargando noticia de cultura...</p>
      </div>
    );
  }
  ...
  if (!noticia) {
    return (
      <div className="categoria-detail-not-found">
        ...
        <p>La noticia que buscas no existe o no es de cultura.</p>
        <Link to="/cultura" className="btn-volver">
          ← Volver a Cultura
        </Link>
      </div>
    );
  ...
  return (
    <article className="categoria-detail">
      <header className="categoria-detail-header cultura-detail-header">
        <Link to="/cultura" className="btn-volver">
          ← Volver a Cultura
        </Link>
