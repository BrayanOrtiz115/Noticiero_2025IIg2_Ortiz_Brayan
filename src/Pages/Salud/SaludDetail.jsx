...
const SaludDetail = () => {
  ...
  useEffect(() => {
    const cargarNoticia = async () => {
      try {
        console.log('📥 Cargando noticia individual de salud:', id);
        const docRef = doc(db, 'noticias', id);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const noticiaData = { id: docSnap.id, ...docSnap.data() };
          console.log('✅ Noticia de salud cargada:', noticiaData.titulo);
          
          // Verificar que la noticia sea de la categoría correcta
          if (noticiaData.categoria !== 'Salud') {
            console.log('❌ Noticia no pertenece a salud');
            setNoticia(null);
          } else {
  ...
  if (!noticia) {
    return (
      <div className="categoria-detail-not-found">
        ...
        <p>La noticia que buscas no existe o no es de salud.</p>
        <Link to="/salud" className="btn-volver">
          ← Volver a Salud
        </Link>
      </div>
    );
  ...
  return (
    <article className="categoria-detail">
      <header className="categoria-detail-header salud-detail-header">
        <Link to="/salud" className="btn-volver">
          ← Volver a Salud
        </Link>
