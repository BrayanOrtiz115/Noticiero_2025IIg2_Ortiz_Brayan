...
const TecnologiaDetail = () => {
  ...
  useEffect(() => {
    const cargarNoticia = async () => {
      try {
        console.log('📥 Cargando noticia individual de tecnología:', id);
        const docRef = doc(db, 'noticias', id);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const noticiaData = { id: docSnap.id, ...docSnap.data() };
          console.log('✅ Noticia de tecnología cargada:', noticiaData.titulo);
          
          // Verificar que la noticia sea de la categoría correcta
          if (noticiaData.categoria !== 'Tecnología') {
            console.log('❌ Noticia no pertenece a tecnología');
            setNoticia(null);
          } else {
  ...
  if (cargando) {
    return (
      <div className="categoria-detail-loading">
        ...
        <p>Cargando noticia de tecnología...</p>
      </div>
    );
  }
  ...
  if (!noticia) {
    return (
      <div className="categoria-detail-not-found">
        ...
        <p>La noticia que buscas no existe o no es de tecnología.</p>
        <Link to="/tecnologia" className="btn-volver">
          ← Volver a Tecnología
        </Link>
      </div>
    );
  ...
  return (
    <article className="categoria-detail">
      <header className="categoria-detail-header tecnologia-detail-header">
        <Link to="/tecnologia" className="btn-volver">
          ← Volver a Tecnología
        </Link>
...
export default TecnologiaDetail;