...
const EducacionPage = () => {
  ...
  useEffect(() => {
    const cargarNoticias = async () => {
      try {
        console.log('📥 Cargando noticias de educación...');
        ...
        console.log('✅ Noticias de educación cargadas:', noticiasData.length);
        setNoticias(noticiasData);
      } catch (error) {
        console.error('❌ Error cargando noticias de educación:', error);
      } finally {
        setCargando(false);
      }
    };
  ...
  if (cargando) {
    return (
      <div className="categoria-loading">
        ...
        <p>Cargando noticias de educación...</p>
      </div>
    );
  }
  ...
  return (
    <div className="categoria-page">
      <div className="categoria-header">
        <div className="categoria-hero educacion-hero">
          <div className="categoria-info">
            <div className="categoria-icon">📚</div>
            <div>
              <h1>Educación</h1>
              <p>Educación y ciencia</p>
            </div>
          </div>
        </div>
      ...
      <div className="categoria-content">
        ...
        {noticiasFiltradas.length === 0 ? (
          <div className="no-noticias">
            ...
            <h3>No hay noticias de educación</h3>
            <p>
              {busqueda 
                ? 'No se encontraron noticias que coincidan con tu búsqueda.'
                : 'No hay noticias de educación publicadas aún.'
              }
            </p>
          </div>
        ) : (
              ...
              <article key={noticia.id} className="noticia-card">
                {noticia.imagen && (
                  <div className="noticia-imagen">
                    ...
                    <div className="noticia-categoria">Educación</div>
                  </div>
                )}
                ...
                <div className="noticia-content">
                  ...
                  <h3 className="noticia-titulo">
                    <Link to={`/educacion/${noticia.id}`}>{noticia.titulo}</Link>
                  </h3>
                  ...
                  <Link to={`/educacion/${noticia.id}`} className="leer-mas">
                    Leer más →
                  </Link>
                </div>
              </article>
