...
const DeportesDetail = () => {
  ...
  useEffect(() => {
    const cargarNoticia = async () => {
      try {
        console.log('📥 Cargando noticia individual de deportes:', id);
        const docRef = doc(db, 'noticias', id);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const noticiaData = { id: docSnap.id, ...docSnap.data() };
          console.log('✅ Noticia de deportes cargada:', noticiaData.titulo);
          
          // Verificar que la noticia sea de la categoría correcta
          if (noticiaData.categoria !== 'Deportes') {
            console.log('❌ Noticia no pertenece a deportes');
            setNoticia(null);
          } else {
  ...
  if (cargando) {
    return (
      <div className="categoria-detail-loading">
        ...
        <p>Cargando noticia de deportes...</p>
      </div>
    );
  }
  ...
  if (!noticia) {
    return (
      <div className="categoria-detail-not-found">
        ...
        <p>La noticia que buscas no existe o no es de deportes.</p>
        <Link to="/deportes" className="btn-volver">
          ← Volver a Deportes
        </Link>
      </div>
    );
  ...
  return (
    <article className="categoria-detail">
      <header className="categoria-detail-header deportes-detail-header">
        <Link to="/deportes" className="btn-volver">
          ← Volver a Deportes
        </Link>
